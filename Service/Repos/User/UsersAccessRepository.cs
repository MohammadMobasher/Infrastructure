...
namespace Service.Repos.User
{
    public class UsersAccessRepository : GenericRepository<UsersAccess>
    {
        ...
        /// <param name="roleId"></param>
        /// <returns></returns>
        public List<UserAccessListViewModel> GetAllUserAccesss(int roleId)
        {
            ...
            if (roleId != 0)
            {
                ...
                if (!_roleRepository.IsAdmin(roleId))
                {

                    var model = TableNoTracking.Where(a => a.RoleId == roleId).ToList();
